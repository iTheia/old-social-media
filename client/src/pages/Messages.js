...
export default function Messages(props) {
  ...
  const printFollows = () =>
    follows.map((follow) => (
      <Container style={{ marginBottom: "10px" }} key={follow._id}>
        <Row>
          <Col md={8} style={{ padding: "0 5px" }}>
            <div className="flex">
              <span className="icon">
                <img src={`/images/${follow.avatar}`} alt="" />
              </span>
          ...
          <Col md={8} style={{ padding: "0" }}>
            <Switch>
              <Route
                ...
                render={() => (
                  <Inbox printFollows={printFollows} follows={follows} />
                )}
              />
              <Route
                path={`${props.match.url}/:room`}
